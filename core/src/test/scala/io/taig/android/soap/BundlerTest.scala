...
import io.taig.android.soap.{ Bundle ⇒ ABundle }
...
class BundlerTest extends Suite {
    def verify[V: bundler.Encoder: bundler.Decoder]( value: V ) = {
        val bundle = implicitly[bundler.Encoder[V]].encode( value )
        implicitly[bundler.Decoder[V]].decode( bundle ) shouldEqual value
        bundle
    }
