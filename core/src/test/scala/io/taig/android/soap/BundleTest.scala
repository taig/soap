...
class BundleTest extends Suite {
    def verify[V: bundle.Encoder: bundle.Decoder]( value: V ) = {
    ...
    it should "support Bundle" in {
        verify[Bundle]( Bundle( "int" ->> 3 :: "string" ->> "asdf" :: HNil ) )
        verify[Bundle]( Bundle( 'int ->> 3 :: 'string ->> "asdf" :: HNil ) )
    }
    ...
    it should "support Option with null values" in {
        Bundle[String]( "value", null ).read[Option[String]]( "value" ) shouldEqual None
    }
    ...
    it should "support sealed trait enums" in {
        verify[Enum]( Enum.A ) shouldEqual Bundle( "value", "A" )
    }
