...
class IntentTest extends Suite {
    def verify[V: intent.Encoder: intent.Decoder]( value: V ) = {
    ...
    it should "support Bundle" in {
        verify[Bundle]( Bundle( "int" ->> 3 :: "string" ->> "asdf" :: "long" ->> 5l :: HNil ) )
        verify[Bundle]( Bundle( 'int ->> 3 :: 'string ->> "asdf" :: 'long ->> 5l :: HNil ) )
    }
    ...
    it should "support sealed trait enums" in {
        verify[Enum]( Enum.B ).getExtras shouldEqual Bundle( "value", "B" )
    }
