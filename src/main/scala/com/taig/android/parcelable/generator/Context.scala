...
trait Context[C <: whitebox.Context]
{
	...
	{
		def extendsFrom[T: TypeTag]: Boolean =
		{
			def getSimpleName( name: String ) = name.replaceFirst( "(?:(?:\\w+\\.)*)(\\w+)(?:\\[.+\\])?", "$1" )

			implDef.impl.parents.exists( parent =>
			{
				try
				{
				...
				catch
				{
					...
					{
						getSimpleName( parent.toString() ) == getSimpleName( typeOf[T].toString )
					}
				}
			} )
		}
