...
object Macro {
    def parcelable( context: whitebox.Context )( annottees: context.Expr[Any]* ): context.Expr[Any] = {
        ...
        def create( c: ClassDef, m: ModuleDef ): Expr[Any] = ( c, m ) match {
            // Object
            case ( null, m: ModuleDef ) ⇒ context.Expr(
                q"""
            ...
            case ( c @ ClassDef( mods, _, _, _ ), m: ModuleDef ) if mods.hasFlag( ABSTRACT ) ⇒ context.Expr(
                q"""
            ...
            // Class, Case Class
            case ( c: ClassDef, m: ModuleDef ) ⇒ context.Expr(
                q"""
